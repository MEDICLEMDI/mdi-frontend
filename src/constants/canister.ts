export const DAB_CANISTER_ID = 'aipdg-waaaa-aaaah-aaq5q-cai';
export const XTC_CANISTER_ID = 'aanaa-xaaaa-aaaah-aaeiq-cai';
export const ICP_CANISTER_ID = 'ryjl3-tyaaa-aaaaa-aaaba-cai';
// export const WICP_CANISTER_ID = 'utozz-siaaa-aaaam-qaaxq-cai';
export const JELLY_CANISTER_ID = 'getti-aiaaa-aaaah-abkkq-cai';
export const ICNS_CANISTER_ID = 'e5kvl-zyaaa-aaaan-qabaq-cai';
export const MDI_CANISTER_ID = 'h4gr6-maaaa-aaaap-aassa-cai';

export const ASSET_CANISTER_IDS = [
  XTC_CANISTER_ID,
  ICP_CANISTER_ID,
  MDI_CANISTER_ID,
  // WICP_CANISTER_ID,
];

export const isICP = (canisterId: string) => canisterId === ICP_CANISTER_ID;
